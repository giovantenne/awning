...
- **8 GB RAM** recommended (setup writes conservative container limits based on detected host RAM/CPU)
...
| `BITCOIN_MEM_LIMIT` / `BITCOIN_CPUS` | Auto-detected, overridable | Bitcoin Core resource limits written by setup based on host RAM/CPU |
| `LND_MEM_LIMIT` / `LND_CPUS` | Auto-detected, overridable | LND resource limits written by setup based on host RAM/CPU |
| `ELECTRS_MEM_LIMIT` / `ELECTRS_CPUS` | Auto-detected, overridable | Electrs resource limits written by setup based on host RAM/CPU |

Setup writes explicit container limits into `.env` using detected host RAM and CPU count. On a shared host, lower them manually after setup.
