...
maybe_require_first_run_disclaimer() {
  ...
  echo ""
  echo -e "  Type ${BOLD}${GREEN}I ACCEPT${NC} to continue:"
...
run_auto_setup() {
  ...
  print_info "Change settings later in Menu>Tools>Setup wizard, or now by typing ${BOLD}w${NC}."
  echo ""
  ...
  local choice
  read -r -p "$(echo -e \
    "  Press ${BOLD}${GREEN}Enter${NC} to start, or type ${BOLD}${YELLOW}'w'${NC} now for the advanced setup wizard: ")" choice </dev/tty
...
generate_rpcauth() {
  ...
  if command -v openssl &>/dev/null; then
    ...
    local result
    result="$(
      _docker run --rm -i python:3-slim python3 - "$password" 2>>"$build_log" <<'PY'
...
auto_initialize_wallet() {
  ...
  local init_response
  init_response="$(
    printf '%s' "$init_payload" | dc_exec -T lnd sh -c \
